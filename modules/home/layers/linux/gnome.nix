...
let
  ...
  extensionLink = ext: let uuid = findUuid ext; in {
    target = "${cfg.homedir}/.local/share/gnome-shell/extensions/${uuid}";
...
in with lib; {
  ...
  options.layers.linux.gnome = {
    enable = mkEnableOption "Standard Gnome configuration";
    homedir = mkOption {};
  };
  ...
  config = mkIf cfg.enable {
    ...
    in {
      ...
      "org/gnome/desktop/background" = {
        picture-uri = "file://${cfg.homedir}/.desktop.jpg";
      ...
      "${extensionsKey}/executor" = {
        ...
        center-active = true;
        center-commands-json = ''{"commands":[{"isActive":true,"command":"echo \"$(roomtemp)°C\"","interval":1,"uuid":"d55e0c0b-fca9-48fa-ae3b-2bfb2e4de580"}]}'';
        center-index = 0;
        left-active = false;
